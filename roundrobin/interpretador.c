...
*	  1			21/10/2019	Versão inicial, baseado no interpretador para o escalonador de tipo prioridade feito por Lucas Damo	
*
***************************************************************************/
#include <stdio.h>	
#include <stdlib.h>
...
void childHandler(void){
	...
	while(TRUE){
		while( ( nBytes = read(p[0], buff, sizeof(Comando)) ) != 0 ){
			printf("Comando %s recebido com prioridade \n", buff->com);
			// Insere na fila
			insereFilaCmd(buff->com);
			 // Nao altera a execução por prioridade
		}
		printf("Executando %d \n", executando);
		...
		if(flagFila != -1){
			...
			if( executando == 0){// nao entendi esse executando ==0, é se n tem ngm executando?
				// Necessario mudar o processo sendo executado
				if (executando != 0){
					printf("Parando processo %d com prioridade \n", executando);
				...
				if(flagFila == 0) insereFilaCmd( cmdBuffer);
				else if (flagFila == 1) insereFilaPid(pidBuffer);
			}
		}
		else{
